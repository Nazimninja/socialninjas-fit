...
let CACHE: any = null                                  // shared across every mounted map
...
function View({ view, levels, onMuscle, selected }: any) {
  return (
    <svg className="bm-v" viewBox={view.vb} role="img">
      {INERT.map(slug => (view.p[slug] || []).map((d: string, i: number) =>
        <path key={slug + i} className="bm-sil" d={d} />))}
      {MUSCLES.map(slug => (view.p[slug] || []).map((d: string, i: number) =>
        <path
          key={slug + i}
...
export default function BodyMap({ load = {}, body = 'male', onMuscle, selected, className = '' }: {
  ...
  const g = paths && ((paths as any)[body] || (paths as any).male)
  return (
